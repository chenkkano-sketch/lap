...
pub fn install_app_menu<R: Runtime>(app: &AppHandle<R>) -> tauri::Result<()> {
    ...
    let about_item =
        MenuItemBuilder::with_id("about", format!("About {}", app.package_info().name))
            .build(app)?;
    let home_item = MenuItemBuilder::with_id("help_homepage", "Nikki Album Home").build(app)?;
...
pub fn handle_menu_event<R: Runtime>(app: &AppHandle<R>, event: MenuEvent) {
    match event.id().0.as_ref() {
        ...
        "help_homepage" => {
            let _ = opener::open(
                option_env!("CARGO_PKG_HOMEPAGE").unwrap_or("https://github.com/chenkkano-sketch/lap"),
            );
        }
        "help_release_notes" => {
        ...
        "help_readme" => {
            let _ = opener::open(format!(
                "{}/blob/main/README.md",
                option_env!("CARGO_PKG_REPOSITORY").unwrap_or("https://github.com/chenkkano-sketch/lap")
            ));
        }
        "help_privacy" => {
            let _ = opener::open(format!(
